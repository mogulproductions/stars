...
contract AccessPassSale is AccessControl {
    ...
    function initTiers() public onlyAdmin {
        ...
        stars.transferFrom(msg.sender, address(this), 160000000 ether);
        tiers[1] = Tier(25, 50 ether, 2000000 ether, false);
        tiers[2] = Tier(100, 25 ether, 500000 ether, false);
        tiers[3] = Tier(250, 10 ether, 100000 ether, false);
        tiers[4] = Tier(2000, 1 ether, 5000 ether, false);
        tiers[5] = Tier(25000000, 0.0003 ether, 1 ether, false);
    }
    ...
    function purchase(uint8 tierNumber) public payable {
        ...
        require(
            msg.value == purchaseTier.passPrice,
            "Incorrect transaction value"
        );
        require(purchaseTier.numPasses > 0, "No passes exist in this tier");
        require(
            !purchaseTier.paused,
            "Purchases from this tier have been paused"
        );

        stars.transfer(msg.sender, purchaseTier.starsPerPass);
        purchaseTier.numPasses -= 1;
    ...
    function removeTiers(uint8[] memory tierNumbers) public onlyAdmin {
        ...
        stars.transfer(withdrawWallet, stars.balanceOf(address(this)));
    }
}
